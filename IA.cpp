...
int main()
{
    ...
    true_begin = Point(100,cube.getPosY());
    true_end = Point(600,cube.getPosY());
    bool first = true;
    // Boucle principale
    while (app.isOpen())
    {
        ...
        if(first) end_point = true_end;
        else end_point = true_begin;

        current = cube.getPos();
        cout << cube.getPosY() << endl;
        if(first && Point(rect.getPosition().x+cube.getHitbox().width/2,cube.getPosY()) == end_point){
            end_point = true_begin;
            begin_point = current;
            begin_node = Node(begin_point);
            first = false;
        }
...
void move_ch(Character& c){
    ...
    c.setPos(Point(c.getPosX()+speed.x,c.getPosY()));
}
